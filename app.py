...
class WorkoutSchema(Schema):
    class Meta:
        fields = ('id', 'name', 'description', 'start_time',
                  'end_time', 'w_type', 'rounds', 'rest_time', 'trainer')
...
def add_user():
    ...
    usr = User(firstname, lastname, email, birth_date,
               address, gender, password_hash)
...
def apple_sign_in_clinet():
    ...
    for key in r.json()['keys']:
        try:
            ...
            decoded = jwt.decode(identityToken, keyPub,
                                 algorithms=alg, audience='com.legend.boxing')
            print(decoded)
        except:
            pass
...
def get_punch_score_for_user(user_id):
    qry = Punch.query.filter(Punch.user_id == user_id).order_by(
        Punch.id.desc()).first()
...
def get_punch_score_with_hr_for_user(user_id):
    p_qry = Punch.query.filter(Punch.user_id == user_id).order_by(
        Punch.id.desc()).first()
    hr_qry = Hr.query.filter(
        Hr.user_id == user_id).order_by(Hr.id.desc()).first()
...
def get_avg_hr_for_user(user_id):
    ...
    qry = Hr.query.filter(Hr.user_id == user_id, Hr.timestamp >
                          time_1h_ago).order_by(Hr.id.desc())
    sum = 0
    cnt = 0
    max = 0
    for h in qry:
...
def add_workout():
    ...
    w = Workout(description, name, start_time, end_time,
                w_type, rounds, rest_time, trainer)
...
def get_day_workout(date):
    ...
    print(t2)
    qry = Workout.query.filter(Workout.start_time >= t1, Workout.start_time <= t2).order_by(
        Workout.start_time.asc()).all()
    return workouts_schema.jsonify(qry)
...
def get_upcoming_workout():
    qry = Workout.query.filter(
        Workout.start_time > datetime.datetime.now()).order_by(Workout.id.asc()).first()
    return workout_schema.jsonify(qry)
...
def get_workout_summary():
    ...
    p_qry = Punch.query.filter(Punch.user_id == user_id, Punch.timestamp >
                               w_start_time, Punch.timestamp < w_end_time).order_by(Punch.id.desc()).first()
    # print(punch_schema.jsonify(p_qry))
    hr_qry = Hr.query.filter(Hr.user_id == user_id, Hr.timestamp >
                             w_start_time, Hr.timestamp < w_end_time).order_by(Hr.id.desc())
...
def add_booking():
    ...
    sub = Subscription.query.filter(Subscription.user_id == user_id, Subscription.start_time < datetime.datetime.now(
    ), Subscription.end_time > datetime.datetime.now()).order_by(Subscription.id.desc()).first()
    try:
        entries_left = sub.entries_left
        if entries_left > 0:
            bookings = Booking.query.filter(
                Booking.workout_id == workout_id).all()
            if len(bookings) < 20:
...
def remove_booking():
    ...
    Booking.query.filter(Booking.workout_id == workout_id,
                         Booking.user_id == user_id).delete()
    sub = Subscription.query.filter(Subscription.user_id == user_id, Subscription.start_time < datetime.datetime.now(
    ), Subscription.end_time > datetime.datetime.now()).order_by(Subscription.id.desc()).first()
...
def end_session():
    ...
    w_qry = Workout.query.filter(Workout.start_time < datetime.datetime.now(
...
def add_subscription():
    ...
    new_sub = Subscription(user_id, start_time,
                           end_time, entries, entries_left)
...
def upcoming_info():
    ...
    for i in users:
        cnt += 1
        item_list.append(dict(firstname=i.firstname, lastname=i.lastname,
                              email=i.email, uid=i.id, dummy='replaceMe'))
    table = CustomerTable(item_list)
    class_info = w_qry.name + ' - ' + \
        w_qry.w_type + ' - ' + str(w_qry.start_time)
...
class SubscriptionForm(FlaskForm):
    ...
    start_date = DateField('Start Date (dd/mm/yyyy - e.g. 21/08/2020',
                           format='%d/%m/%Y', validators=[DataRequired()])
    entries = SelectField('Entries', choices=[
                          '1', '8', '12', '999'], validators=[DataRequired()])
    ...
    if request.method == 'POST':
        try:
            start_date = datetime.datetime.strptime(
                request.form.get('start_date'), '%d/%m/%Y')
            email = request.form.get('email')
            entries = request.form.get('entries')

            usr = User.query.filter(User.email == email).order_by(
                User.id.desc()).first()
            user_id = usr.id

...
def get_upcoming_workout():
    qry = Workout.query.filter(Workout.end_time > datetime.datetime.now() - datetime.timedelta(hours=2)).order_by(Workout.start_time.asc()).first()
    return workout_schema.jsonify(qry)
...
def connected_users():
    ...
    for k in bag_map:
        if bag_map[k] != "":
            ...
            tup = (k, user_schema.jsonify(usr).content)
            print(tup)
            users.append(tup)
...
def upcoming_info():
    w_qry = Workout.query.filter(Workout.end_time > datetime.datetime.now() - datetime.timedelta(hours=2)).order_by(Workout.start_time.asc()).first()
    w_id = w_qry.id
